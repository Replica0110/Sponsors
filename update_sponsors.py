...
class Config:
    ...
    HTML_TEMPLATE: str = "templates/sponsor_template.html"
...
class HtmlExporter:
    ...
    def __init__(self, filepath: str, template_path: str = None):
        self.filepath = Path(filepath)
        self.template_path = Path(template_path or config.HTML_TEMPLATE)
    ...
    def _load_template(self) -> str:
        ...
        if self._template_cache is None:
            if not self.template_path.exists():
                raise FileNotFoundError(f"HTML 模板文件不存在: {self.template_path}")
...
def main() -> None:
    ...
    try:
        ...
        html_exporter = HtmlExporter(config.HTML_FILE, config.HTML_TEMPLATE)
